...
class Validator {
  ...
  _validateStringField(key, field, value, errors, path) {
    ...
    if (field.min_length != null && value.length < field.min_length) {
      errors.push(this._createError("Length of " + key + " should be " + field.min_length + " or more.", path));
    }
    if (field.max_length != null && value.length > field.max_length) {
      errors.push(this._createError("Length of " + key + " should be " + field.min_length + " or less.", path));
    }
  }
  _validateNumberField(key, field, value, errors, path) {
    if (field.min != null && value < field.min) {
      errors.push(this._createError("Value of " + key + " should be " + field.min + " or greater.", path));
    }
    if (field.max != null && value > field.max) {
      errors.push(this._createError("Value of " + key + " should be " + field.max + " or less.", path));
    }
  }
  _validateIntegerField(key, field, value, errors, path) {
    if (value != Math.floor(value)) {
      errors.push(this._createError(key + " should be an integer.", path));
    }
    this._validateNumberField(key, field, value, errors, path);
  }
  _validateBooleanField(key, field, value, errors, path) {
    if (typeof(value) != 'boolean') {
      errors.push(this._createError(key + " should be true or false.", path));
    }
  }
  ...
  _validateObjectField(key, field, value, errors, path) {
    if (typeof(value) != 'object') {
      errors.push(this._createError(key + " should be an object.", path));
      return;
    }
    if (field.rule != null) {
  ...
  _validate (rule, data, errors, parentPath) {
    for (let key in rule) {
      ...
      if (data[key] == null) {
        if (field.required) {
          errors.push(this._createError(key + " is required.", path));
        }
        continue;
      }
